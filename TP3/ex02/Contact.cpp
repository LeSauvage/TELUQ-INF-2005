...
Contact::Contact(const string& name, const string& sexe, const int& age,
                 const string& phone)
    : name_(name), sexe_(sexe), age_(age), phone_(phone), currentBus_(nullptr) {
...
void Contact::display() const {
  ...
  if (currentBus_ != nullptr) {
    cout << "Current Bus: " << currentBus_->getName() << ":"
         << currentBus_->getType() << endl;
  }
}

void Contact::setBus(Bus* bus) { this->currentBus_ = bus; }
